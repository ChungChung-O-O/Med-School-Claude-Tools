...
- **Anatomy relationships** — innervation, action, nerve roots, attachments, blood supply, drainage. Prefer cloze for these and bias toward cloze when the source is anatomy. Cloze multiple linked facts in one sentence with separate deletions so each is tested independently, e.g. `The {{c1::axillary nerve}} (roots {{c2::C5-C6}}) innervates {{c3::deltoid}} and teres minor; injury causes loss of {{c4::arm abduction}}.`
  - For *visual* identification (naming a structure pointed to on an image), text cloze is the wrong tool — use an **Image Occlusion** card (native notetype in Anki 23.10+). These CAN be built programmatically via AnkiConnect (`storeMediaFile` + `addNote` with model `Image Occlusion`; `Occlusion` field holds `{{cN::image-occlusion:rect:left=:top=:width=:height=}}` entries, coords as 0–1 fractions). Get coordinates from the source: for PDF/PPTX with selectable text labels use `fitz` `get_text("words")` for exact label boxes; for flat images estimate and let the user nudge. Build into `MedSchool` with the tag schema. See the `reference-anki-image-occlusion` memory for the full format. Cannot be derived from text alone — needs the actual image.
